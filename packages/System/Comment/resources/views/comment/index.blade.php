@extends('comment::layout.main')

@section('content')
    <div class="row">
        <div class="col-xs-12">
            <div class="portlet light bordered">

                <div class="portlet-body">
                    {!! Form::open(['class' => 'comment-list']) !!}
                    {!! Form::hidden('_method', '') !!}
                    <table class="table table-striped table-bordered table-hover table-checkable order-column data-table-checkable" data-delete-link="{{ $ajax_delete_link  }}" data-toggle-link="{{ $ajax_toggle_link  }}" >
                        <thead>
                        <tr>
                            <th><input type="checkbox" class="group-checkable" data-set=".data-table-checkable .checkboxes" /></th>
                            <th> Tên thành viên </th>
							<th> Nội dung </th>
							<th> Sắp Xếp </th>
                            <th> Trạng thái kiểm duyệt </th>
                            <th> Ngày tạo </th>
                            <th> Hành động </th>
                        </tr>
                        </thead>
                        <tbody>
                        @foreach($listComment as $comment )
                            <tr class="odd gradeX">
                                <td><input type="checkbox" name="id[]" class="checkboxes" value="{{ $comment->id }}" /> </td>
                                <td class="item-name">
                                    {{ $comment->user_name }}
                                </td>
                                <td align="center">{!!  $comment->message  !!}</td>
                                <td align="center">
                                    <a href="#" class="x-editable" id="order" data-pk="{{ $comment->id }}" data-type="text" data-url="{{ route("admin.comment.updates") }}" data-title="Số thứ tự">{{ $comment->order }}</a>
                                </td>
                                <td align="center"><a href="javascript:;" class="toggle-item" data-property="status" ><i class="fa {{ $comment->status == 1 ? 'fa-check' : 'fa-times' }} fa-lg"></i></a></td>
                                <td align="center">{{ $comment->created_at }}</td>
                                <td align="center">
                                    {!! ViewHelper::button('Xóa ', null, 'link', ['icon' => 'icon-trash', 'class' => 'delete-item', 'size'=>'xs', 'color' => 'red', 'data-toggle' => 'confirmation']) !!}
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
                    </table>
                    {!! Form::close() !!}
                </div>
            </div>
        </div>
    </div>
@endsection